...
COMMON_TRAIN_FORMAT = [('epoch', 'E', 'int'), ('step', 'S', 'int'), 
                       ('loss', 'L', 'float'), ('total_time', 'T', 'time')]

COMMON_EVAL_FORMAT = [('epoch', 'E', 'int'), ('step', 'S', 'int'), 
                       ('overall_accuracy', 'A', 'float'), 
                       ('collision_accuracy', 'CA', 'float'), ('nocollision_accuracy', 'NCA', 'float'), 
                       ('total_time', 'T', 'time')]
...
class Logger(object):
    def __init__(self, log_dir, use_tb, use_wandb, name=None, project=None, cfg=None):
        ...
        self._trajopt_mg = MetersGroup(log_dir / 'trajopt.csv',
    ...
    def log(self, key, value, step):
        ...
        if key.startswith('train'):
            ...
            mg = self._eval_mg
        elif key.startswith('trajopt'):
            mg = self._trajopt_mg
        elif key.startswith('collision'):
            mg = self._coll_mg
        elif key.startswith('per_plot'):
            mg = self._per_plot_mg
        else:
            mg = self._plot_mg
        mg.log(key, value)
    ...
    def dump(self, step, ty=None):
        ...
        if ty is None or ty == 'plot':
            self._plot_mg.dump(step, 'plot')
        if ty is None or ty == 'per_plot':
            self._per_plot_mg.dump(step, 'per_plot')
        if ty is None or ty == 'trajopt':
            self._trajopt_mg.dump(step, 'trajopt')
        if ty is None or ty == 'collision':
            self._coll_mg.dump(step, 'collision')
